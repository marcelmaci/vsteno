...
$rules_list = array( 
    ...
    "R2" => array( "C1", "V2", "D1", "?" ),   // "" means mandatory
    "R3" => array( "C2", "V1", "D3", "" ),
    "R4" => array( "C2", "V2", "D4", "?" ),
    "R5" => array( "C3", "V1", "D5", "" ),
    "R6" => array( "C3", "V2", "D6", "?" ),
...
// show result
echo "<h2>TOKENS</h2>"; foreach ($token_groups_string as $key => $string) echo "$key: $string<br>";
echo "<h2>VOWELS</h2>"; foreach ($vowel_groups_string as $key => $string) echo "$key: $string<br>";
...
foreach ($rules_list as $key => $rule) {
    ...
    $rules_list_string[$key] = mb_strtolower("\"($group1)($vowel)$optional($group2)\" => \"$1[#$distance]$2$3\";") . " // $key|$combination_key: $group1_key#$vowel_key:$distance#$group2_key";
}

// show result (copy it to VSTENO model)
echo "<h2>RULES</h2>"; foreach ($rules_list_string as $key => $string) echo "$string<br>";

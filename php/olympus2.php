    ...
    if (($_SESSION['user_privilege'] > 1) || (($_SESSION['user_privilege'] == 1) && ($_SESSION['model_standard_or_custom'] === "custom"))) {
      ...
      // Create connection
        ...
        if ($_POST['delete_entry'] !== "delete_yes") {  // update entry
            ...
            $word_id = $conn->real_escape_string($_POST['word_id']);
            $sql = "DELETE FROM $olympus WHERE word_id='$word_id'";
        }
        
        $result = $conn->query($sql);
        
        if ($result === TRUE) {
            if ($_POST['delete_entry'] === "delete_yes") { echo "<p><b>QUERY</b><br>$sql</p>"; die_more_elegantly("<p>Der Eintrag in Olympus ($olympus) wurde gelöscht.</p>"); }
            else echo "<p>Die Daten wurden in Olympus ($olympus) geschrieben.</p>";
            echo "<p><b>QUERY</b><br>$sql</p>";
        } else {
            die_more_elegantly("<p>Fehler beim Schreiben des Eintrages in Olympus ($olympus).</p>");
        }
        ...
        echo "<p><a href='olympus1.php?word_id=$word_id'><button>ändern</button></a> <a href='olympus.php'><button>->Olympus</button></a></p>";

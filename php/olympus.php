...
if (($_SESSION['user_logged_in']) && ($_SESSION['user_privilege'])) {
    if (($_SESSION['user_privilege'] > 1) || (($_SESSION['user_privilege'] == 1) && ($_SESSION['model_standard_or_custom'] === "custom"))) {
        ...
        echo "
        <h1>Olympus</h1>
        <p>Hier können Sie Ihre Einträg in Olympus ($olympus) bearbeiten.</p><p>Wählen Sie einen der untenstehenden Einträge aus, um ihn zu bearbeiten.</p>
        <h1>Einträge ($olympus)</h1>";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row != null) {
                echo "<a href='olympus1.php?word_id=" . $row['word_id'] . "'>" . $row['word']  . "</a> ";
                $row = $result->fetch_assoc();
